    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        json.forEach(function(j) {
          ...
          sources[name] = {
            url: j.url,
            name: j.name, // Убрано принудительное переименование 2160 в 720
...
segments: file.segments,
  ...
  console.log(Lampa.Arrays.getKeys(data.quality));
      if (Lampa.Arrays.getKeys(data.quality).length) {
        var qualities = Lampa.Arrays.getKeys(data.quality).map(Number).sort((a, b) => b - a); // Сортировка по убыванию для максимального качества
        var maxQuality = qualities[0]; // Выбор максимального качества
        data.url = data.quality[maxQuality];
  ...
  first.subtitles_call = json_call.subtitles_call || json.subtitles_call;
              // Убрано добавление VAST рекламы
  ...
  clarification: all[id] ? true : false
    ...
    Lampa.Lang.add({
      ...
      lampac_balanser_timeout: { //
        ...
        uk: 'Джерело буде автоматически переключено через <span class="timeout">10</span> секунд.',
...
if (Lampa.Manifest.app_digital >= 177) {
  var balansers_sync = [
    "filmix", "filmixtv", "fxapi", "rezka", "rhsprem", "lumex", "videodb",
    "collaps", "collaps-dash", "hdvb", "zetflix", "kodik", "ashdi",
    "kinoukr", "kinotochka", "remux", "iframevideo", "cdnmovies",
    "anilibria", "animedia", "animego", "animevost", "animebesst",
    "redheadsound", "alloha", "animelib", "moonanime", "kinopub",
    "vibix", "vdbmovies", "fancdn", "cdnvideohub", "vokino",
    "rc/filmix", "rc/fxapi", "rc/rhs", "vcdn", "videocdn", "mirage",
    "hydraflix", "videasy", "vidsrc", "movpi", "vidlink", "twoembed",
    "autoembed", "rgshows", "smashystream", "autoembed", "pidtor",
...
// БЛОК ПОДСКАЗОК ПОЛНОСТЬЮ УДАЛЁН
