(function() {
  ...
  window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
    window.rch_nws[hostkey].typeInvoke('http://z01.online', function() {
      ...
      client.invoke("RchRegistry", JSON.stringify({
        version: 149,
        host: location.host,
        rchtype: Lampa.Platform.is('android') ? 'apk' : window.rch_nws[hostkey].type,
  ...
  function component(object) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        network.silent(account(url), function(json) {
          ...
          if (json.life) {
            ...
            _this4.lifeSource().then(_this4.startSource).then(resolve)["catch"](function() {
    ...
    this.parseJsonDate = function(str, name) {
      ...
      try {
        ...
        var elems = [];
        var items = doc.querySelectorAll(name);
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          try {
            ...
            var s = item.getAttribute('s');
            var e = item.getAttribute('e');
            
            if (e) data.episode = parseInt(e);
            if (s) data.season = parseInt(s);
            if (item.textContent) data.text = item.textContent.trim();
            data.active = item.classList.contains('active');
    ...
    this.filter = function(filter_items, choice) {
      ...
      // ДОБАВЛЯЕМ ПУНКТ "ИСТОЧНИК" В НАЧАЛО МЕНЮ
      if (filter_sources && filter_sources.length > 1) {
        select.push({
          title: Lampa.Lang.translate('settings_rest_source'),
          subtitle: sources[balanser] ? sources[balanser].name : balanser,
          stype: 'source',
          onSelect: function() {
            // При выборе открываем список источников
    ...
    this.getEpisodes = function(season, call) {
      if (!object.movie.name || !object.movie.id) {
        call([]);
        return;
      }
      ...
      var tmdburl = 'tv/' + tmdb_id + '/season/' + season;
  ...
  // ===== УЛУЧШЕННЫЙ КОНТРОЛЛЕР ДЛЯ ВЫБОРА ИСТОЧНИКА =====
  Lampa.Controller.listener.follow('toggle', function (event) {
    if (event.name !== 'select') return;
    
    var $filterTitle = $('.selectbox__title');
    
    // Проверяем, что открыто меню фильтра онлайн-плагина
    if ($filterTitle.length !== 1 || $filterTitle.text() !== Lampa.Lang.translate('title_filter')) return;
    
    // Ищем активность онлайн-плагина
    var activeActivity = Lampa.Activity.active();
    if (!activeActivity || activeActivity.component !== 'lampac_z01') return;
    
    // Ищем кнопку выбора источника
    var $sourceBtn = $('.simple-button--filter.filter--sort');
    if ($sourceBtn.length !== 1) return;
    
    // Получаем текущее название источника
    var sourceName = $('div', $sourceBtn).text();
    if (!sourceName) return;
    
    // Добавляем пункт "Источник" в меню
    var $selectBoxItem = Lampa.Template.get('selectbox_item', {
      title: Lampa.Lang.translate('settings_rest_source'),
      subtitle: sourceName
    });
    
    // Делаем его кликабельным для навигации
    $selectBoxItem.addClass('selector');
    
    // При нажатии Enter открываем выбор источника
    $selectBoxItem.on('hover:enter', function () { 
      // Закрываем текущее меню
      Lampa.Select.close();
      // Даем время на закрытие и открываем выбор источника
      setTimeout(function() {
        $sourceBtn.trigger('hover:enter');
      }, 50);
    });
    
    // Вставляем пункт первым в меню
    var $firstItem = $('.selectbox-item').first();
    if ($firstItem.length) {
      $firstItem.before($selectBoxItem);
    } else {
      $('.selectbox__body').prepend($selectBoxItem);
    }
    
    // ОБЯЗАТЕЛЬНО: Обновляем навигацию с новым элементом
    var $allSelectors = $('body > .selectbox').find('.selector');
    Lampa.Controller.collectionSet($allSelectors);
    
    // Также обновляем коллекцию в скролле
    var $scrollBody = $('body > .selectbox').find('.scroll__body');
    if ($scrollBody.length) {
      Lampa.Controller.collectionSet($scrollBody);
    }
    
    // Фокусируемся на первом элементе (источнике)
    setTimeout(function() {
      Lampa.Controller.collectionFocus($selectBoxItem[0]);
    }, 10);
  });
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '1.1.0',
      name: 'Онлайн',
      description: 'Просмотр видео из онлайн источников',
      component: 'lampac_z01',
      onContextMenu: function onContextMenu(object) {
        return { name: Lampa.Lang.translate('lampac_watch') };
      },
    ...
    <div class="full-start__button selector view--online lampac--button">
      <svg width="128" height="128" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="10" y="10" width="108" height="108" rx="8" fill="none" stroke="currentColor" stroke-width="10"/>
