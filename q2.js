(function() {
  ...
  function component(object) {
    ...
    function getBalanserName() {
      if (balanser && sources[balanser]) {
        return sources[balanser].name;
      }
      return balanser || 'Источник';
    }
    ...
    this.startSource = function(json) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        if (filter_sources.length) {
          ...
          // Проверка, что выбранный балансер существует
          if (!sources[balanser]) {
          ...
          reject('Нет доступных источников');
        }
      }.bind(this));
    };
    ...
    this.createSource = function() {
      var _this4 = this;
      return new Promise(function(resolve, reject) {
        var url = _this4.requestParams(Defined.localhost + 'lite/events');
    ...
    this.selected = function(filter_items) {
      ...
      filter.chosen('sort', [sources[balanser] ? sources[balanser].name : getBalanserName()]);
    };
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          element.mark = function() {
            ...
            _this8.watched({
              balanser: balanser,
              balanser_name: Lampa.Utils.capitalizeFirstLetter(sources[balanser] ? sources[balanser].name.split(' ')[0] : getBalanserName()),
    ...
    this.noConnectToServer = function(er) {
      var balanser_name = getBalanserName();
      var html = Lampa.Template.get('lampac_does_not_answer', {});
      html.find('.online-empty__buttons').remove();
      html.find('.online-empty__title').text(Lampa.Lang.translate('title_error'));
      html.find('.online-empty__time').text(er && er.accsdb ? er.msg : Lampa.Lang.translate('lampac_does_not_answer_text').replace('{balanser}', balanser_name));
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    };
