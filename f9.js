(function () {
                ...
                if (response && response.status === 200 && response.json) {
                    ...
                    if (data && data.player_links) {
                        if (data.player_links.playlist && data.player_links.playlist.length > 0) {
                            this.showSeasons(data.player_links.playlist, object, data.title || object.title);
                        } else if (data.player_links.movie && data.player_links.movie.length > 0) {
                            this.playMovie(data.player_links.movie, object);
                        } else {
    ...
    function showAuthPanel() {
        ...
        var dialog = Lampa.Dialog.create({
            html: html,
            width: 650,
            height: 500,
            onBack: true
    ...
    function initPlugin() {
        ...
        try {
            ...
            console.error('Filmix init error:', err);
        }
    }
