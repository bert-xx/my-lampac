(function() {
  ...
  function component(object) {
    ...
    this.requestParams = function(url) {
      ...
      // ОБМАН СЕРВЕРА: отправляем запрос как будто мы хотим 720p
      query.push('prefer_quality=720');
      query.push('max_quality=720');
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        json.forEach(function(j) {
          ...
          // ОБМАН: показываем пользователю настоящее качество, но серверу говорим что мы хотим 720
          var displayName = j.name;
          // Восстанавливаем 4K в названии если сервер его скрыл
          if (displayName.includes('720') && (j.name.includes('2160') || j.name.includes('4K'))) {
            displayName = j.name.replace('720', '2160').replace('HD', '4K');
          }
          
          sources[name] = {
            url: j.url,
            name: displayName, // Показываем настоящее качество пользователю
            originalName: j.name, // Сохраняем оригинальное название
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            json.online.forEach(function(j) {
              var name = balanserName(j);
              // ОБМАН: показываем настоящее качество
              var displayName = j.name;
              if (displayName.includes('720') && (j.name.includes('2160') || j.name.includes('4K'))) {
                displayName = j.name.replace('720', '2160').replace('HD', '4K');
              }
    ...
    this.setDefaultQuality = function(data) {
      console.log('Доступные качества:', Lampa.Arrays.getKeys(data.quality));
      
      if (Lampa.Arrays.getKeys(data.quality).length) {
        // ВЫБИРАЕМ НАИВЫСШЕЕ КАЧЕСТВО ВМЕСТО 720p
        var qualities = Object.keys(data.quality)
          .map(function(q) { return parseInt(q); })
          .filter(function(q) { return !isNaN(q); })
          .sort(function(a, b) { return b - a; }); // Сортировка по убыванию
        
        if (qualities.length > 0) {
          var highestQuality = qualities[0];
          
          // Отдаем предпочтение 4K/2160, потом 1080, потом 720
          var preferredQuality = qualities.find(function(q) {
            return q === 2160 || q === 1080 || q === 720;
          }) || highestQuality;
          
          console.log('Выбираем качество:', preferredQuality + 'p');
          data.url = data.quality[preferredQuality];
        }
        
        this.orUrlReserve(data);
        
        // Очищаем ссылки от " or "
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          if (element.quality) {
            ...
            // ВЫВОДИМ ВСЕ ДОСТУПНЫЕ КАЧЕСТВА В ИНТЕРФЕЙС
            var qualityKeys = Object.keys(element.quality).map(function(q) { 
              return parseInt(q); 
            }).filter(function(q) { 
              return !isNaN(q); 
            }).sort(function(a, b) { 
            ...
            if (qualityKeys.length > 0) {
              element.quality = qualityKeys.map(function(q) { return q + 'p'; }).join(' / ');
            } else {
              element.quality = '';
            }
          }
          ...
          if (serial && !episode) {
            ...
            var imagePath = episode ? episode.still_path : object.movie.backdrop_path;
            var imageSize = window.innerWidth > 480 ? 'w780' : 'w300';
            img.src = Lampa.TMDB.image('t/p/' + imageSize + imagePath);
            images.push(img);
          }
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function(episode) {
            ...
            if (episode.still_path) {
              ...
              img.src = Lampa.TMDB.image('t/p/w780' + episode.still_path);
              images.push(img);
            } else {
              loader.remove();
              image.append('<div class="online-prestige__episode-number">' + ('0' + episode.episode_number).slice(-2) + '</div>');
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '',
      name: 'Online',
      description: 'Плагин для онлайн просмотра',
      component: 'lampac_z01',
      onContextMenu: function onContextMenu(object) {
        return {
          name: Lampa.Lang.translate('lampac_watch'),
          description: 'Смотреть онлайн в высоком качестве'
        };
      },
    ...
    // ИЗМЕНЕНА ИКОНКА С "Z" НА "Online"
    var button = `
      <div class="full-start__button selector view--online lampac--button" data-subtitle="${manifst.description}">
        <svg width="128" height="128" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect x="8" y="8" width="112" height="112" rx="32" fill="white" stroke="#2886fb" stroke-width="12"/>
          <text x="50%" y="50%" text-anchor="middle" dominant-baseline="central" font-family="Arial, sans-serif" font-size="36" font-weight="bold" fill="#2886fb">Online</text>
        </svg>
    ...
    // УДАЛЕНЫ ВСЕ РЕКЛАМНЫЕ ПОДСКАЗКИ И ХИНТЫ
    // Оставлен только полезный контент
